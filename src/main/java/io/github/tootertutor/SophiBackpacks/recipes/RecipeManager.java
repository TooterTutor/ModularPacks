package io.github.tootertutor.SophiBackpacks.recipes;
 ...
 *
 * Notes:
 * - Backpacks and modules contain unique UUIDs, so we cannot rely on static recipe
 * outputs; we swap the result on CraftItemEvent.
 * - SHIFT craft is blocked for these recipes to avoid mass-creating unique items
...
public final class RecipeManager implements Listener {
    ...
    public RecipeManager(SophiBackpacksPlugin plugin) {
        this.plugin = plugin;
    ...
    public void onPrepareCraft(PrepareItemCraftEvent e) {
        ...
        // Keep result stable/preview-only here; real UUID output is created in CraftItemEvent.
    ...
    public void onCraft(CraftItemEvent e) {
        ...
        if (e.isShiftClick()) {
            e.setCancelled(true);
            player.sendMessage(Text.c("&cCraft one at a time for SophiBackpacks items."));
            return;
        }
        ...
        if (dyn.kind == DynamicKind.BACKPACK && result.hasItemMeta()) {
            ...
            if (meta != null) {
                ...
                String typeId = meta.getPersistentDataContainer().get(plugin.keys().BACKPACK_TYPE, PersistentDataType.STRING);
                if (idStr != null && typeId != null) {
                    UUIDUtils.Parsed parsed = UUIDUtils.tryParse(idStr);
                    if (parsed != null) {
                        plugin.repo().ensureBackpackExists(parsed.uuid(), typeId, player.getUniqueId(), player.getName());
                    }
                }
            }
        }
    ...
    public void onSmith(SmithItemEvent e) {
        ...
        Keys keys = plugin.keys();
        String idStr = result.getItemMeta().getPersistentDataContainer().get(keys.BACKPACK_ID, PersistentDataType.STRING);
        String newType = result.getItemMeta().getPersistentDataContainer().get(keys.BACKPACK_TYPE, PersistentDataType.STRING);
        ...
        if (oldType == null) {
            ...
            if (base != null && base.hasItemMeta()) {
                oldType = base.getItemMeta().getPersistentDataContainer().get(keys.BACKPACK_TYPE, PersistentDataType.STRING);
            }
        }
        if (oldType == null)
            oldType = newType;
