...
public final class OpenSubcommand implements Subcommand {
    ...
    public OpenSubcommand(SophiBackpacksPlugin plugin) {
        this.plugin = plugin;
    ...
    public void execute(CommandContext ctx) {
        ...
        if (rows.isEmpty()) {
            ctx.sender().sendMessage(Component.text("No backpacks of type " + pick.typeId + " found in DB for " + target.getName() + "."));
            return;
        }

        int idx = pick.index1 - 1;
        if (idx < 0 || idx >= rows.size()) {
            ctx.sender().sendMessage(Component.text("Index out of range. Max for " + pick.typeId + " is " + rows.size() + "."));
            return;
        }
